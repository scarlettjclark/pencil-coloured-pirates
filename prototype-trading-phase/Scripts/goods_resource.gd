## This class bundles together the various types of GoodsResource in this game so that
## they can be easily compared and passed in a type safe manner

class_name GoodsResource
...
## Returns whether the GoodsResource stored in this object are enough to pay for the
## cost of the parameter GoodsResource.
func has_cost(cost : GoodsResource) -> bool:
	var result := true
	for type in goods_dict.keys():
		result = result and (goods_dict[type] >= cost.goods_dict[type])
	return result

func add(to_add : GoodsResource) -> void:
	for type in goods_dict.keys():
		goods_dict[type] += to_add.goods_dict[type]

func spend(to_spend : GoodsResource) -> void:
	for type in goods_dict.keys():
		goods_dict[type] -= to_spend.goods_dict[type]

func has_room_for_more(cost : GoodsResource, current_load : GoodsResource) -> bool:
	var combined = GoodsResource.new()
	combined.add(cost)
